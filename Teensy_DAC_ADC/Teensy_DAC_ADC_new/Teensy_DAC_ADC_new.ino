...
void loop() {
  ...
  if(received>=0 && received <=7){
      ...
      while(flag==false){
        ...
        if(DAC_value>MAX_DAC) DAC_value=MAX_DAC;
        analogWrite(DACPin, DAC_value);
        DAC_values[0][step]=DAC_value;
        
        Serial.print("step = ");Serial.println(step);
        Serial.print("code_DAC = ") ;Serial.println(DAC_value);
        
        delay(1000) ;
        DAC_values[1][step]= adc->adc0->analogRead(ADCPin); 
        Serial.print("code_ADC = ") ;Serial.println(DAC_values[1][step]) ;
        
        step++;
        if(DAC_value==MAX_DAC) flag=true;        
        DAC_value=DAC_value+DAC_step;

      } 

       for (int i = 0; i < step; ++i) {
        Serial.print( DAC_values[0][i]);
        Serial.print(",");
        Serial.println( DAC_values[1][i]);
        }
        step=0;
        DAC_value=0;
        flag=false;
     ...
     Serial.println("incorrect command try again");
  }
 }    
}
